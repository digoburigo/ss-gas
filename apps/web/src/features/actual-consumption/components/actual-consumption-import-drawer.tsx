...
type ActualConsumptionImportDrawerProps = {
  open: boolean;
...
export function ActualConsumptionImportDrawer({
  open,
  onOpenChange,
}: ActualConsumptionImportDrawerProps) {
  const [uploadedFile, setUploadedFile] = useState<File | null>(null);
  const [result, setResult] = useState<UploadResult | null>(null);
  const [excludedRows, setExcludedRows] = useState<Set<number>>(new Set());
  const [editingRow, setEditingRow] = useState<number | null>(null);
  const [rowEdits, setRowEdits] = useState<
    Map<number, { qdrValue?: number; notes?: string | null }>
  >(new Map());
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const response = await api.gas["actual-consumption"].upload.post({
        fileBase64: base64,
      });
  ...
  const confirmMutation = useMutation({
    mutationFn: async (rows: InterpretedRow[]) => {
      const validRows = rows
        ...
        .map((r) => {
          ...
          return {
            ...
            qdrValue: edits?.qdrValue ?? r.qdrValue,
            source: r.source,
      ...
      const response = await api.gas["actual-consumption"].confirm.post({
        rows: validRows,
        importLog: {
          totalRows,
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await api.gas["actual-consumption"].template.get();
      if (response.error) {
      ...
      a.href = url;
      a.download = "template-consumo-real.xlsx";
      a.click();
  ...
  const startEditing = (rowNumber: number, row: InterpretedRow) => {
    ...
    if (!rowEdits.has(rowNumber)) {
      setRowEdits((prev) => {
        ...
        next.set(rowNumber, { qdrValue: row.qdrValue, notes: row.notes });
        return next;
      });
    }
  };
  ...
  const updateRowEdit = (
    ...
    setRowEdits((prev) => {
      ...
      if (field === "qdrValue") {
        ...
        next.set(rowNumber, {
          ...current,
          qdrValue: Number.isNaN(parsed) ? 0 : parsed,
        });
      } else {
  ...
  const getRowQdrValue = (row: InterpretedRow): number => {
    const edits = rowEdits.get(row.rowNumber);
    return edits?.qdrValue ?? row.qdrValue;
  };
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="flex flex-col overflow-hidden p-0 sm:max-w-[900px]">
        <SheetHeader className="border-b px-6 py-4">
          <SheetTitle>Importar Consumos</SheetTitle>
          <SheetDescription>
            Faca upload de uma planilha Excel para importar registros de consumo
            real em lote. A IA interpretara os dados da planilha.
          </SheetDescription>
        </SheetHeader>
                ...
                <div className="space-y-4">
                  <div className="flex flex-col items-center justify-center py-6">
                    ...
                    <p className="text-muted-foreground mt-1 text-sm">
                      Os registros de consumo foram salvos com sucesso.
                    </p>
                  </div>
                ...
                >
                  <div className="border-input hover:border-primary/50 hover:bg-accent/50 flex w-full max-w-md cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed p-8 transition-colors">
                    <input
                      ...
                      className="hidden"
                      id="actual-consumption-upload"
                    />
                    <label
                      htmlFor="actual-consumption-upload"
                  ...
                  {result.columnMapping && (
                    <div className="bg-muted/50 rounded-md p-3 text-sm">
                      ...
                      <div className="text-muted-foreground flex flex-wrap gap-x-4 gap-y-1">
                        ...
                        {result.columnMapping.qdrValue && (
                          <span>
                            Consumo:{" "}
                            <strong>{result.columnMapping.qdrValue}</strong>
                          </span>
                        )}
                        {result.columnMapping.source && (
                          <span>
                            Fonte:{" "}
                            <strong>{result.columnMapping.source}</strong>
                          </span>
                        )}
                        ...
                        {result.rows.map((row) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <TableCell className="text-right text-sm font-mono">
                                {isEditing ? (
                                  <Input
                                    ...
                                    defaultValue={qdrValue?.toLocaleString(
                                      "pt-BR",
                              ...
                              <TableCell className="text-muted-foreground max-w-[120px] text-sm">
                                {isEditing ? (
                                  <Input
                                    type="text"
